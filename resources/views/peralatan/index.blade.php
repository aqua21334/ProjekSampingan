...
<div class="dashboard">
    ...
    <main class="main-content">
        <div class="content">
            <!-- Data Peralatan Section -->
            <div class="permintaan-section" style="max-width:900px; margin:auto;">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-extrabold text-4xl text-gray-900">Daftar Peralatan</h2>
                    <a href="{{ route('peralatan.create') }}" class="bg-blue-600 text-gray-900 px-4 py-2 rounded shadow hover:bg-blue-700">Tambah Peralatan</a>
                </div>
                <!-- Search Form -->
                <form method="GET" action="" class="flex gap-4 mb-6">
                    <input type="text" name="kode_bmn" value="{{ request('kode_bmn') }}" placeholder="Cari Kode BMN" class="border rounded px-3 py-2 w-1/3" />
                    <input type="text" name="nama_peralatan" value="{{ request('nama_peralatan') }}" placeholder="Cari Nama Peralatan" class="border rounded px-3 py-2 w-1/3" />
                ...
                <table style="width:100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background:#f0f0f0;">
                            ...
                            <th style="padding:10px; border:1px solid #ddd;">Kode BMN</th>
                            <th style="padding:10px; border:1px solid #ddd;">Nama Peralatan</th>
                            <th style="padding:10px; border:1px solid #ddd;">Tanggal Kalibrasi</th>
                            <th style="padding:10px; border:1px solid #ddd;">Status Kalibrasi</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="padding:10px; border:1px solid #ddd;">{{ $peralatan->kode_bmn }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $peralatan->nama_peralatan }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $peralatan->tanggal_kalibrasi }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $peralatan->status_kalibrasi }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">
                                <a href="{{ route('peralatan.edit', $peralatan->kode_bmn) }}" style="color:#007bff;">Edit</a> |
                                <form action="{{ route('peralatan.destroy', $peralatan->kode_bmn) }}" method="POST" style="display:inline;">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="6" style="padding:10px; text-align:center;">Tidak ada data peralatan.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
