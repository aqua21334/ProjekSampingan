...
<div class="dashboard">
    ...
    <main class="main-content">
        <div class="content">
            <!-- Data Permintaan Section -->
            <div class="permintaan-section" style="max-width:900px; margin:auto;">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-extrabold text-4xl text-gray-900">Daftar Permintaan Layanan</h2>
                    <a href="{{ route('permintaan.create') }}" class="bg-blue-600 text-gray-900 px-4 py-2 rounded shadow hover:bg-blue-700">Tambah Permintaan</a>
                </div>
                <!-- Search Form -->
                <form method="GET" action="" class="flex gap-4 mb-6">
                    <input type="text" name="id_permintaan" value="{{ request('id_permintaan') }}" placeholder="Cari ID Permintaan" class="border rounded px-3 py-2 w-1/3" maxlength="20" />
                    <input type="text" name="jenis_permintaan" value="{{ request('jenis_permintaan') }}" placeholder="Cari Jenis Permintaan" class="border rounded px-3 py-2 w-1/3" maxlength="50" />
                ...
                <table style="width:100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background:#f0f0f0;">
                            ...
                            <th style="padding:10px; border:1px solid #ddd;">ID Permintaan</th>
                            <th style="padding:10px; border:1px solid #ddd;">Pemohon</th>
                            <th style="padding:10px; border:1px solid #ddd;">Jenis Permintaan</th>
                            <th style="padding:10px; border:1px solid #ddd;">Status</th>
                            <th style="padding:10px; border:1px solid #ddd;">Keterangan</th>
                            <th style="padding:10px; border:1px solid #ddd;">Tanggal Permintaan</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="padding:10px; border:1px solid #ddd;">{{ $permintaan->id_permintaan }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $permintaan->pemohon }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $permintaan->jenis_permintaan }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $permintaan->status }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $permintaan->keterangan }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">{{ $permintaan->created_at }}</td>
                            <td style="padding:10px; border:1px solid #ddd;">
                                <a href="{{ route('permintaan.edit', $permintaan->id_permintaan) }}" style="color:#007bff;">Edit</a> |
                                <form action="{{ route('permintaan.destroy', $permintaan->id_permintaan) }}" method="POST" style="display:inline;">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="8" style="padding:10px; text-align:center;">Tidak ada data permintaan.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
