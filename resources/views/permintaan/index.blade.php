        ...
        <div class="content">
            <div class="card-section">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="title">Daftar Permintaan Layanan</h2>
                    <a href="{{ route('permintaan.create') }}" class="btn-add"><i data-lucide="plus-circle"></i> Tambah Permintaan</a>
                </div>

                <!-- Search -->
                <form method="GET" class="search-form">
                    <input type="text" name="id_permintaan" placeholder="Cari ID Permintaan" value="{{ request('id_permintaan') }}" />
                    <input type="text" name="jenis_permintaan" placeholder="Cari Jenis Permintaan" value="{{ request('jenis_permintaan') }}" />
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="actions">
                                    <a href="{{ route('permintaan.edit', $permintaan->id_permintaan) }}" class="edit"><i data-lucide="edit"></i> Edit</a>
                                    <form action="{{ route('permintaan.destroy', $permintaan->id_permintaan) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="delete" onclick="return confirm('Yakin hapus permintaan?')"><i data-lucide="trash-2"></i> Hapus</button>
                                    </form>
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="8" class="text-center">Tidak ada data permintaan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
