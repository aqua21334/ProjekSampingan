...
<body>
    ...
    <div class="form-container">
        <h1>Tambah PNBP</h1>
        <form action="{{ route('pnpb.store') }}" method="POST">
            @csrf
            <div class="form-group">
                <label for="kode_billing">Kode Billing</label>
                <input type="text" id="kode_billing" name="kode_billing" placeholder="Masukkan kode billing" required value="{{ old('kode_billing') }}">
                @if ($errors->has('kode_billing'))
                    <div style="color: #d32f2f; font-size: 14px; margin-top: 4px;">
                        {{ $errors->first('kode_billing') }}
                    </div>
                @endif
            </div>
            <div class="form-group">
                <label for="pelanggan">Pelanggan</label>
                <input type="text" id="pelanggan" name="pelanggan" placeholder="Masukkan nama pelanggan" required value="{{ old('pelanggan') }}">
            ...
            <div class="form-group">
                ...
                <input type="number" id="rupiah" name="rupiah" placeholder="Masukkan nominal rupiah" required value="{{ old('rupiah') }}" step="0.01">
